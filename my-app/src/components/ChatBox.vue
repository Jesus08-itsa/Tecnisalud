    ...
    <div class="container">
      <h1>AI Chat Boot</h1>
        ...
        <div class="inputContainer">
          <input v-model="currentMessage" type="text" class="messageInput" placeholder="Preguntame todo..."/>
          <button @click="sendMessage(currentMessage)" class="askButton"> Enviar
          </button>
        </div>
     </div>
        <!--Boton desplegable-->
          <div class="chatbox__button">
            <button><img src="../assets/chat-dots.svg" /></button>
          </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    async sendMessage(message) {
      ...
      await axios
        .post('http://localhost:5173/chatbot', {
          message: message,
        })
       .then((response) => {
        this.messages.push({
